document.addEventListener('DOMContentLoaded', function () {
    ...
    //Codigo para arrastrar horizontalmente la seccion de reseñas
    let presionar = false;
    let posicionInicial = 0;

    //funcion para realizar cuando el usuario presione el mouse
    reseñas.addEventListener('mousedown', function (event) {
        if (event.target.closest('#formularioReseña')) {
            // Si el clic fue dentro del formulario, no bloquear el comportamiento por defecto
            return;
        }
